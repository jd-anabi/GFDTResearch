...
if __name__ == '__main__':
    ...
    if nd:
        ...
        # time interval
        t_interval = [0, 500]
        t = np.linspace(t_interval[0], t_interval[1], 10000000)
        ...
        plt.xlim(t_interval[0] + 250, t_interval[1])
        plt.ylim(np.min(w_sol.y[0][int(len(t) / 2):]) - 0.25, np.max(w_sol.y[0][int(len(t) / 2):]) + 0.25)
        plt.show()

        #print(fdt_hf.fdt_test([w_sol.y[0]], 1000000, 1000 / 1000000, 1 / sp.constants.k))
    else:
        # read non-dimensional hair cell from csv file
